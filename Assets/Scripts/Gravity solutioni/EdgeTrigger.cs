...
public class EdgeTrigger : MonoBehaviour
{
    ...
    public CubeFace targetFace; // La cara a la que se moverá el jugador

    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            GravityPlayer gravity = other.GetComponent<GravityPlayer>();
            if (gravity != null && targetFace != null)
            {
                gravity.AlignToFace(targetFace);
            }
        }
    }
}
